...
class TrafficCalculator(BaseMethod):
    ...
    def get_trafic_calculation(self, request_area_geojson):
        ...
        return {"buildings": selected_buildings.reset_index(drop=True).to_crs(self.city_crs), 
                "stops": nearest_stops.reset_index(drop=True).to_crs(self.city_crs), 
                "routes": house_stop_routes.reset_index(drop=True).to_crs(self.city_crs)}