...
class ServicesClusterization(BaseMethod):
    ...
    def get_clusters_polygon(self, service_types, area_type = None, area_id = None, geojson = None, 
        ...
        services = pd.concat([services_normal, services_outlier]).set_crs(self.city_crs).to_crs(self.city_crs )
        df_clusters = df_clusters.set_crs(self.city_crs).to_crs(self.city_crs)

        return {"polygons": df_clusters, "services": services}
